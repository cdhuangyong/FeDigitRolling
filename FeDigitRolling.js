...
FeDigitRolling.prototype = {
    ...
    createElementObject:function(){
        ...
        var fwidth = hasDot ? (w + (w / 2) / (len-1)  ) : w;
        var dotWidth = fwidth / 2;
        var fheight = this.height;

        var aw = hasDot ? (fwidth * (len - 1) + dotWidth) :  fwidth * (len);
        ...
        for (var i = 0; i < len ; i++) {
            ...
            this.createOrUpdateObj(i,c,hasDot,dotWidth,fwidth,fheight,fontSize,startx,group);
            startx += (c == "." ? dotWidth : fwidth) + this.gap - 0.05 * i;
        }
        this.len = len;
        return group
    },
    ...
    createOrUpdateObj:function(index,c,hasDot,dotWidth,fwidth,fheight,fontSize,startx,parent){
        ...
        if(this.block1[index] ){
            ...
            el.style.width = (c == "." ? dotWidth : fwidth) + "px";
